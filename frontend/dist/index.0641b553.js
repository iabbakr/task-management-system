...
})({"hrBMw":[function(require,module,exports,__globalThis) {
...
document.getElementById("register-btn").addEventListener("click", async (e)=>{
    ...
    const response = await (0, _apiJs.register)({
        username,
        email,
        password
    });
    if (response.token) {
        ...
        showTaskSection();
    } else alert(response.error || "Registration failed!");
});
document.getElementById("login-btn").addEventListener("click", async (e)=>{
    e.preventDefault();
    const email = document.getElementById("email").value;
    const password = document.getElementById("password").value;
    const response = await (0, _apiJs.login)({
        email,
        password
    });
    if (response.token) {
        ...
        showTaskSection();
        loadTasks();
    } else alert(response.message || "Login failed!");
});
document.getElementById("task-form").addEventListener("submit", async (e)=>{
    ...
    const response = await (0, _apiJs.createTask)(token, {
        title,
        description,
        deadline,
        priority
    ...
    if (response._id) {
        ...
        loadTasks();
    } else alert(response.error || "Failed to add task!");
});
...
const loadTasks = async ()=>{
    const tasks = await (0, _apiJs.getTasks)(token);
...
},{}]},["hrBMw","bNKaB"], "bNKaB", "parcelRequire94c2")
